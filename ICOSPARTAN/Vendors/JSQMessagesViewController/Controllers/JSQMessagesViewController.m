...
@property (NS_NONATOMIC_IOSONLY, readonly, copy) NSString *jsq_currentlyComposedMessageText;
...
@property (NS_NONATOMIC_IOSONLY, readonly) BOOL jsq_inputToolbarHasReachedMaximumHeight;
...
@property (NS_NONATOMIC_IOSONLY, readonly) BOOL jsq_isMenuVisible;
...
#warning OJO ANDRES AQUI
    
    // Load Wallpaper Image
    /*wallpName = [[NSUserDefaults standardUserDefaults]
                             stringForKey:@"wallpName"];
    // Set Chat Wallpaper image
    _chatBkgImage.image = [UIImage imageNamed:wallpName];*/
    

    
    _jsq_isObserving = NO;
    
    _toolbarHeightConstraint.constant = kJSQMessagesInputToolbarHeightDefault;
    
    _collectionView.dataSource = self;
    _collectionView.delegate = self;
    
    _inputToolbar.delegate = self;
    _inputToolbar.contentView.textView.placeHolder = NSLocalizedStringFromTable(@"New Message", @"JSQMessages", @"Placeholder text for the message input text view");
    _inputToolbar.contentView.textView.delegate = self;
    _inputToolbar.backgroundColor = [UIColor lightGrayColor];
    
    _senderId = @"JSQDefaultSender";
    _senderDisplayName = @"JSQDefaultSender";
    
    _automaticallyScrollsToMostRecentMessage = true;
