    ...
    if (self) {
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self
                                                 selector:@selector(cleanup)
                                                     name:NOTIFICATION_USER_LOGGED_OUT object:nil];
...
- (void)viewDidLoad {
    ...
    // Set navBar title
	self.title = @"Agregar contactos al grupo";
    ...
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"Cancelar" style:UIBarButtonItemStylePlain target:self action:@selector(cancelPressed)];
    ...
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"Hecho" style:UIBarButtonItemStylePlain target:self action:@selector(donePressed)];
...
- (void)donePressed {
	if ([selection count] == 0) {
        [ProgressHUD showError:@"No hay contacto seleccionado."]; return;
    }
