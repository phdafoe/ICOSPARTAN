...
- (void)viewDidAppear:(BOOL)animated {
    ...
    // ...if not, open the LoginVC
        ...
        UINavigationController *navCont = (UINavigationController *)[storyboard instantiateViewControllerWithIdentifier:@"NavigationController"];
...
- (void)inviteUser:(NSDictionary *)user {
	...
	if (([user[@"emails"] count] != 0) && ([user[@"phones"] count] != 0)) {
		UIActionSheet *action = [[UIActionSheet alloc] initWithTitle:nil delegate:self cancelButtonTitle:@"Cancel"
            ...
            otherButtonTitles:@"Invite by Email",
                              @"Invite by SMS",
                              nil];
        ...
        [ProgressHUD showError:@"This contact does not have enough information to be invited."];
    }
}
...
- (void)sendEmailToUser:(NSDictionary *)user {
	...
	else [ProgressHUD showError:@"Please configure your email account."];
}
// MAIL DELEGATE
...
- (void)sendSMS:(NSDictionary *)user {
	...
	if ([MFMessageComposeViewController canSendText]) {
        ...
        [ProgressHUD showError:@"Your device cannpt send SMS messages!"];
    }
}
