...
- (void)saveUser {
	if ([_nameTxt.text length] != 0) {
		...
		[user saveInBackgroundWithBlock:^(BOOL succeeded, NSError *error) {
			if (error == nil) {
				[ProgressHUD showSuccess:@"Your details has been saved!"];
			
            } else [ProgressHUD showError:@"Network error."];
		}];
    
    } else {
       [ProgressHUD showError:@"Please type your Name"];
    }
}
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex {
	...
	if (buttonIndex != actionSheet.cancelButtonIndex) {
        ...
        WelcomeVC *welcomeVC = [self.storyboard instantiateViewControllerWithIdentifier:@"WelcomeVC"];
        navCont = [[UINavigationController alloc]initWithRootViewController:welcomeVC];
